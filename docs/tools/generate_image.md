...
- When enabled, `generate_image` stays top-level so the model receives its schema directly rather than discovering it through the `xd://` catalog. A tool-gated system directive keeps native image routing authoritative even when resumed conversation history contains earlier programmatic image attempts.
- For an explicit image-generation or image-edit request, invoke `generate_image` directly without asking for redundant confirmation or substituting `write`, node REPLs, handcrafted SVG, HTML/canvas output, browser rendering, screenshots, or rasterization unless the user explicitly requested programmatic/vector graphics.
