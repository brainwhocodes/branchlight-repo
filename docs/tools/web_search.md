...
- Subprocesses / native bindings
  ...
  - This fallback can start a Chromium process and create its browser-profile lifecycle. On first browser use it can also download Chromium into the OMP browser cache unless `OMP_BROWSER_EXECUTABLE_PATH` or a system Chromium installation is available. The search adapter itself uses no native binding.
