...
test("covers Work and Code session supervision", async () => {
  ...
  await expect(page.getByRole("option", { name: "Use Compact Fixture from alternate" })).toBeVisible();
  await expect(page.getByRole("option", { name: "Use Fast Fixture from fixture" })).toHaveCount(0);
  await modelProviderFilter.selectOption("fixture");
  await modelSearch.fill("Fast Fixture");
  const fastModel = page.getByRole("option", { name: "Use Fast Fixture from fixture" });
  ...
  await expect(fastModel).toHaveAttribute("aria-selected", "true");
  ...
  await expect(page.getByText("result.txt")).toBeVisible();
