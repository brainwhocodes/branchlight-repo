...
describe("applyProviderGlobalsFromSettings", () => {
	...
	it("reapplies valid web and image provider globals from cwd-scoped settings", () => {
		...
		const imageExcludeSpy = vi.spyOn(imageGen, "setExcludedImageProviders").mockImplementation(() => {});
		...
		applyProviderGlobalsFromSettings({
			get(
				...
				const values: Record<string, unknown> = {
					...
					"providers.imageExclude": ["antigravity", 42, "openrouter"],
				};
				return values[path];
			},
		});
		...
		expect(imageExcludeSpy).toHaveBeenCalledWith(["antigravity", "openrouter"]);
	});
});
